...
const GatchaCard = ({ monstre, flipOnHover = false, disableClickFlip = false, onClick }) => {
    const [isFlipped, setIsFlipped] = useState(false);
    
    const initialSrc = monstre?.nom ? `/assets/monsters/${monstre.nom}.png` : `/assets/monsters/Default_Monster.png`;
    const [currentImage, setCurrentImage] = useState(initialSrc);

    // Reset image when monster prop changes
    useEffect(() => {
        setCurrentImage(monstre?.nom ? `/assets/monsters/${monstre.nom}.png` : `/assets/monsters/Default_Monster.png`);
    }, [monstre]);
    ...
    return (
        ...
        >
            ...
            <div className={`card-flipper ${isFlipped ? 'flipped' : ''}`}>
                {/* RECTO */}
                <div className={`card-face card-front element-${elementClass}`} style={{ backgroundImage: `url(${currentImage})` }}>
                    <img 
                ...
                {/* VERSO */}
                <div className={`card-face card-back element-${elementClass}`} style={{ backgroundImage: `url(${currentImage})` }}>
                    <div className="card-overlay" />
