...
const MOCK_INVENTORY = Array.from({ length: 24 }).map((_, i) => ({
    ...
    image: null,
    skills: [
        { name: "Frappe Rapide", description: "Inflige 120% dégâts physiques.", cost: 10, cooldown: 3 },
        { name: "Bouclier Élémentaire", description: "Réduit les dégâts de 30% pendant 2 tours.", cost: 25, cooldown: 8 },
        { name: "Ultimatum", description: "Une attaque dévastatrice qui consomme tous les HP restants.", cost: 0, cooldown: 120 }
    ]
}));
...
const Inventory = () => {
    ...
    return (
        <div className={`inventory-container ${theme} ${selectedMonster ? 'blur-background' : ''}`}>
            ...
            {selectedMonster && (
                <div className={`detail-overlay ${isTransitioning ? 'fade-in' : 'visible'}`} onClick={(e) => {
                    ...
                    <div className="detail-content glass-panel">
                        ...
                        <div className="detail-right">
                            ...
                            <div className="detail-tags">
                                <span className={`tag rank-${selectedMonster.rang}`}>{selectedMonster.rang}</span>
                                <span className={`tag element-${selectedMonster.element}`}>{selectedMonster.element}</span>
                            </div>
                            
                            <div className="detail-stats-grid">
                                <div className="stat-box"><span>HP</span> <strong>{selectedMonster.stats?.hp}</strong></div>
                                <div className="stat-box"><span>ATK</span> <strong>{selectedMonster.stats?.atk}</strong></div>
                                <div className="stat-box"><span>DEF</span> <strong>{selectedMonster.stats?.def}</strong></div>
                                <div className="stat-box"><span>VIT</span> <strong>{selectedMonster.stats?.vit}</strong></div>
                            </div>

                            <div className="detail-lore">
                                <h3>Origine</h3>
                                <p>{selectedMonster.lore || "L'origine de cette créature reste un mystère pour les érudits."}</p>
                            </div>
