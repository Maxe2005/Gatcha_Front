...
const Gacha = () => {
    ...
    const handleInvoke = async () => {
        ...
        try {
            ...
            const response = await invocationApi.get('/api/invocation/global-invoque/'+user.username);
    ...
    return (
        <Box className={`gacha-page ${theme === 'dark' ? 'theme-dark' : 'theme-divine'} ${isRevealed ? 'revealed' : 'entering'}`} sx={{ flexGrow: 1, minHeight: '100vh', background: 'var(--bg-primary)' }}>
            ...
            <Container maxWidth="md" sx={{ mt: 4, textAlign: 'center', pb: 4 }}>
                <Typography variant="h3" gutterBottom sx={{ color: 'var(--text-primary)', fontFamily: 'Cinzel, serif' }}>
                ...
                >
                    {loading ? <CircularProgress size={24} color="inherit" /> : 'INVOQUER'}
                </Button>
